...
create or replace function public.request_account_deletion(confirmation_phone text)
...
begin
  ...
  normalized_phone := regexp_replace(trim(coalesce(confirmation_phone, '')), '\D', '', 'g');

  select users.phone_e164
  into account_phone_e164
  from public.users
  where users.id = current_user_id;

  normalized_account_phone := regexp_replace(coalesce(account_phone_e164, ''), '\D', '', 'g');
  ...
  if normalized_phone = '' or normalized_account_phone = '' or normalized_phone <> normalized_account_phone then
    ...
    raise exception 'phone confirmation does not match account phone';
  end if;
