drop function if exists public.apply_email_job_delivery_result(uuid, int, int, int, jsonb, text);
drop function if exists public.claim_email_job_for_delivery(uuid);

drop policy if exists email_job_recipients_admin_select_org on public.email_job_recipients;
drop index if exists public.email_job_recipients_job_status_idx;
drop table if exists public.email_job_recipients;
